...
public class RotateCube extends JFrame {
	...
	class PaintPanel extends JPanel {
		...
		//Ligning 2.19 i Eberly reading material
		...
		PaintPanel() {
			...
			cube[6]=new V3(3,6,1);
			cube[7]=new V3(3,6,3);
			//Adderer punkter fra cube arrayet
			for (int i=0; i<cube.length; i++){
				c=c.add(cube[i]);
			}
			
			//Finder centrum af cuben
		...
		public void paintComponent(Graphics g) {
			...
			//Transformerer punkterne til hvert t(tid)
			for (int i=0; i<cube.length; i++){
				//cube[i]=cube[i].sub(c);
